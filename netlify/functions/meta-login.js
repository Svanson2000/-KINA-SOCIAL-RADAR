...
function signedState(){
  ...
  const payload=Buffer.from(JSON.stringify({t:Date.now(),n:crypto.randomBytes(12).toString('hex')})).toString('base64url');
...
exports.handler=async()=>{
  ...
  if(!state) return {statusCode:302,headers:{Location:HOME+'?meta=missing_secret','Cache-Control':'no-store'},body:''};
